...
set -g status-right "#[fg=#{@thm_yellow},bg=#{@thm_surface_0}]   #{cpu_percentage} #{@separator} #{battery_icon} #{battery_percentage} #{battery_remain} #[fg=#{@thm_fg},bg=#{@thm_surface_0}]#{@separator} #{flow_status} #{@separator} 󰚭 #{uptime} #[fg=#{@thm_fg},bg=#{@thm_surface_0},nobold,noitalics,nounderscore]#{@separator} #{world_clock_status} #[fg=#{@thm_fg},bg=#{@thm_surface_0},nobold,noitalics,nounderscore]#{@separator} #[fg=#{@thm_maroon}]  %H:%M:%S "
