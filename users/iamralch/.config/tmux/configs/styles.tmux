...
set -g status-left "#[fg=#{@thm_blue}] #{keyboard_layout} #[fg=#{@thm_fg}]#{@separator} #{?@is_upterm_session,#[fg=#{@thm_red}],#[fg=#{@thm_blue}]}#S #[fg=#{@thm_yellow}]#{?@aws_profile_session,  ,}#[fg=#{@thm_fg}]#{@separator}"
set -g status-left-length 100
# Status Right Format and Styles
set -g status-right "#{@separator}#[fg=#{@thm_yellow}]   #{cpu_percentage} #[fg=#{@thm_fg}]#{@separator} #[fg=#{@thm_green}]#{battery_icon} #{battery_percentage} #{battery_remain} #[fg=#{@thm_fg}]#{@separator} #{pomodoro_status} #[fg=#{@thm_fg}]#{@separator} 󰚭 #{uptime} #[fg=#{@thm_fg},nobold,noitalics,nounderscore]#{@separator} #{world_clock_status} #[fg=#{@thm_fg},nobold,noitalics,nounderscore]#{@separator} #[fg=#{@thm_maroon}]  %H:%M:%S "
